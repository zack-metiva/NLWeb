...
from web_server_utils import get_logger
...
async def handle_mcp_request(query_params, body, send_response, send_chunk, streaming=False):
    ...
    """
    logger = get_logger()
    logger.info(f"Handling MCP request (streaming={streaming})")
    log(f"Handling MCP request (streaming={streaming})")
        ...
        if body:
            try:
                ...
                if not streaming:
                    ...
                    # Print the attributes of result
                    ...
                    class MCPStreamHandler:
                        ...
                        async def write(self, chunk, end_response=False):
                            ...
                            try:
                                ...
                                log(f"Error in MCPStreamHandler.write: {str(e)}")
                                self.closed = True
                    ...
                    try:
                        ...
                        log(f"Error in streaming request: {str(e)}\n{traceback.format_exc()}")
