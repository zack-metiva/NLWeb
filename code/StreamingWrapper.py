...
class HandleRequest():
    ...
    async def do_GET(self):
        ...
        log(f"[{request_id}] Received GET request for path: {self.path}")
        try:
            await self._start_sse_response()
            
            if not self.connection_alive:
                log(f"[{request_id}] Connection lost before starting query handling")
                return
            ...
            log(f"[{request_id}] Connection error during request handling: {str(conn_err)}")
            self.connection_alive = False
            # Connection errors are expected and don't need to generate an error response
            return
        except Exception as inner_e:
            if self.connection_alive:
                log(f"[{request_id}] Error during request handling: {str(inner_e)}")
                error_msg = f"Error processing request: {str(inner_e)}"
                await self.send_error_response(500, error_msg)
            else:
                log(f"[{request_id}] Error after connection was already lost: {str(inner_e)}")
            return
    ...
    async def send_error_response(self, status_code, message):
        ...
        try:
            ...
            self.connection_alive = False
            log(f"Connection lost while sending error response: {str(ssl_err)}")
        except Exception as e:
            log(f"Critical error during error handling: {str(e)}")
    ...
    async def write_stream(self, message):
        ...
        try:
            ...
            self.connection_alive = False
            log(f"Connection lost while writing to stream: {str(e)}")
            # Don't re-raise - consumer should check connection_alive flag
        except Exception as e:
            log(f"Error writing to stream: {str(e)}")
            self.connection_alive = False
            # Don't re-raise - consumer should check connection_alive flag
    ...
    async def _start_sse_response(self):
        ...
        try:
            ...
            self.connection_alive = False
            log(f"Connection lost before sending headers: {str(e)}")
