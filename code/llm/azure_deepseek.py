...
def get_deepseek_azure_endpoint():
...
def get_deepseek_azure_api_key():
...
def get_deepseek_azure_api_version():
...
def get_deepseek_client():
    ...
    with _client_lock:
        if deepseek_client is None:
            ...
            endpoint = get_deepseek_azure_endpoint()
            api_key = get_deepseek_azure_api_key()
            api_version = get_deepseek_azure_api_version()
...
def clean_deepseek_response(content):
...
async def get_deepseek_completion(prompt, json_schema, model=None, temperature=0.7, timeout=8):
...
Your response must exactly match the following JSON schema: {json.dumps(json_schema)}
Only output the JSON object itself, with no markdown formatting, no explanations, and no additional text."""
    ...
    try:
        ...
        result = clean_deepseek_response(content)
