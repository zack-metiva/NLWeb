...
async def fulfill_request(method, path, headers, query_params, body, send_response, send_chunk):
    ...
    try:
        ...
        if path == "/" or path == "":
            ...
            # Return OAuth client configuration (public info only)
            oauth_config = getattr(CONFIG, 'oauth', {})
            public_config = {
                'google': {
                    'enabled': oauth_config.get('google', {}).get('enabled', False),
                    'client_id': oauth_config.get('google', {}).get('client_id', '')
                },
                'facebook': {
                    'enabled': oauth_config.get('facebook', {}).get('enabled', False),
                    'app_id': oauth_config.get('facebook', {}).get('client_id', '')  # Facebook uses client_id in config
                },
                'microsoft': {
                    'enabled': oauth_config.get('microsoft', {}).get('enabled', False),
                    'client_id': oauth_config.get('microsoft', {}).get('client_id', '')
                },
                'github': {
                    'enabled': oauth_config.get('github', {}).get('enabled', False),
                    'client_id': oauth_config.get('github', {}).get('client_id', '')
                }
            }
            await send_response(200, {'Content-Type': 'application/json'})
            await send_chunk(json.dumps(public_config), end_response=True)
            return
