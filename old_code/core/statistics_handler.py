...
class StatisticsHandler():
    ...
    def _load_templates(self) -> List[Dict]:
        ...
        try:
            import os
            templates_path = os.path.join(os.path.dirname(__file__), 'statistics_templates.txt')
                    ...
                    if line[0].isdigit() and '.' in line:
                        ...
                        if len(parts) == 2:
                            ...
                            if '{' in template_text:
                                ...
                                vars_str = template_text[pattern_end:].strip()
                            else:
                                # No variables specified, use the whole text as pattern
    ...
    def _load_dcid_mappings(self) -> Dict:
        ...
        try:
            import os
            mappings_path = os.path.join(os.path.dirname(__file__), 'dcid_mappings.json')
    ...
    async def score_template_match(self, user_query: str, template: Dict) -> Tuple[str, int]:
        """Score how well a template matches the user's query using LLM."""
        prompt = f"""
        ...
        Return only a single integer between 0 and 100, and a justificaiton for the score.
        """
        
        try:
            # Use a simple schema that accepts any string
            simple_schema = {"score": "string", "justification": "string"}
            response = await ask_llm(prompt, simple_schema, level="low")
            print(f"Template: {template['pattern']} Response: {response}")
            
            # Extract the score from the response
            score = 0
            if isinstance(response, dict):
                # Get the response string
                response_str = str(response.get('score', '')).strip()
            else:
                response_str = str(response).strip()
            
            # Try to parse as integer
            if response_str.isdigit():
                score = int(response_str)
            else:
            ...
            return (template['id'], min(max(score, 0), 100))
        except Exception as e:
    ...
    async def extract_variables_and_places(self, query: str, template: Dict) -> Tuple[List[str], List[str]]:
        """Extract variables and places from the query based on the template."""
        prompt = f"""
        User query: "{query}"
        ...
        1. Variable names (e.g., "population", "median income", "number of veterans", "age")
        2. Place names (e.g., "San Francisco county", "Los Angeles", "Texas", "US", "United States")
        
        Special cases:
        - If the query says "across US counties" or "across counties", extract "US" as the place
        - If the query says "across states", extract "US" as the place
        - If no specific place is mentioned but it says "counties" or "states", assume "US"
        ...
        try:
            ...
            print(f"\nExtracting variables/places with template: {template['pattern']}")
            response = await ask_llm(prompt, return_structure, level="low")
            print(f"Extraction response: {response}")
            
            if isinstance(response, dict):
                return response.get('variables', []), response.get('places', [])
            else:
                # Fallback: try to parse as JSON string
                result = json.loads(str(response))
                return result.get('variables', []), result.get('places', [])
        except Exception as e:
            logger.error(f"Error extracting variables and places: {e}")
            return [], []
    ...
    async def map_to_dcids(self, variables: List[str], places: List[str]) -> Tuple[List[str], List[str]]:
        ...
        for var in variables:
            ...
            if dcid:
                ...
                async def get_variable_dcid(prompt=prompt, var=var):
                    ...
                    if response and response.strip() != "UNKNOWN":
                        print(f"    ✓ Variable '{var}' -> LLM found DCID '{response}'")
                        return (var, response)
                    else:
                        print(f"    ✗ Variable '{var}' -> NO MAPPING FOUND (LLM returned: {response})")
                        return (var, None)
        ...
        for result in variable_results:
            if isinstance(result, tuple):
                ...
                variable_dcids.append(result)
        
        if failed_variables:
            print(f"  ⚠️  Failed to map variables: {failed_variables}")
