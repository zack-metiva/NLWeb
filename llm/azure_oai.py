...
class LLMClients:
    ...
    def get_azure_openai_client(cls):
        if cls.azure_openai_client is None:
            ...
            if cls.useasync:
                cls.azure_openai_client = AsyncAzureOpenAI(
    ...
    # @classmethod
    # def get_azure_openai_async_client(cls):
    #     if cls.azure_openai_async_client is None:
    #         endpoint = get_azure_openai_endpoint()
    #         cls.azure_openai_client = AsyncAzureOpenAI(
    #             azure_endpoint=endpoint,
    #             api_key=get_azure_openai_api_key(),
...
def get_structured_completion_async(prompt, json_schema, model_name="gpt-4.1-mini", temperature=0.7, timeout=8):
    if (LLMClients.useasync):
        return get_azure_openai_completion_async(prompt, json_schema, model_name, temperature, timeout)
    else:
...
async def get_azure_openai_completion_async(prompt, json_schema, model_name="gpt-4.1-mini", temperature=0.7, timeout=8):
    client = LLMClients.get_azure_openai_client()
    system_prompt = f"""Provide a response that matches this JSON schema: {json.dumps(json_schema)}"""
